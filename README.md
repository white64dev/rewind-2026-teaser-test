...
- **Owner's Manual.** Clicking the manual on the desk (or its keyboard button) opens a 3D book reader (`src/book.js`, loaded on demand). Each sheet is a bone-skinned box that curls as it turns. Pages are `public/manual/p01–p20.webp`, split from the 1976 manual's two-page scans (the first scan holds the back and front covers), and the PDF is linked for download.
